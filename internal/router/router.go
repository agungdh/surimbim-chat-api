...
func New(cfg *config.Config, hub *websocket.Hub) http.Handler {
	...
	loginLimiter := mw.NewRateLimiter(10, time.Minute)
	r.With(loginLimiter.Middleware()).Post("/api/login", handler.Login(db, ts, cfg))
	r.Post("/api/logout", handler.Logout(ts))

	r.Get("/ws", handler.WsHandler(hub, cfg))

	r.With(mw.RequireAuth(ts)).Get("/api/users", handler.ListUsers(db))
	r.With(mw.RequireAuth(ts)).Get("/api/me", handler.Me(db))
	r.With(mw.RequireAuth(ts)).Get("/api/conversations", handler.ListConversations(db))
